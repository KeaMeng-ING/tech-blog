...
export default async function ReposPage() {
  ...
  return (
    <div className="px-10 md:px-16 py-10">
      {/* ╭┈➤ HERO SECTION */}
      <section className="relative w-full h-105 md:h-130 rounded-2xl overflow-hidden mb-12">
        {/* Background Image */}
        <img
        ...
        <div className="relative z-10 h-full flex flex-col justify-center px-8 md:px-16 max-w-4xl">
          ...
          <p className="text-gray-400 max-w-xl text-sm md:text-base">
            Explore the most impactful and trending repositories shaping
            <br />
      ...
      <section className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {repos.map((repo: any, index: number) => (
          <div
            key={index}
            className="bg-linear-to-br from-white/5 to-white/0 border border-white/10 rounded-2xl p-6 flex flex-col justify-between min-h-65 hover:scale-[1.02] hover:border-purple-400/30 transition"
          >
            {/* TOP */}
            <div>
            ...
            <Link
              href={repo.url}
              target="_blank"
              className="mt-6 block text-center py-3 rounded-full bg-linear-to-r from-purple-500 to-indigo-500 text-sm font-medium hover:opacity-90 transition"
            >
              View Repository →
            </Link>
          </div>
